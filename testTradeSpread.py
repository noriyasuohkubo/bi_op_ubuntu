...
start = datetime(2018, 7, 19,22)
start_stp = int(time.mktime(start.timetuple()))

end = datetime(2018, 7, 21 )
...
def get_redis_data(db):
    ...
    close_data, high_data, low_data, label_data, time_data, price_data , predict_time_data, predict_score_data , end_price_data \
        = [], [], [], [], [], [], [], [], []
    spread_data = []
    spread0 , spread1, spread2,spread3 , spread4, spread5over = 0,0,0,0,0,0
    up =0
    same =0
    ...
    spread_total = spread1 + spread2 + spread3 + spread4 + spread5over + spread0
    ...
    print("spread5over: ", spread5over / spread_total)
...
if __name__ == "__main__":
    ...
    r = redis.Redis(host=host, port=6379, db=db_no, decode_responses=True)
    tradeReults = r.zrangebyscore(symbol + db_suffix + "_TRADE" + db_suffix_trade, start_stp, end_stp, withscores=True)

    spread0, spread1, spread2, spread3, spread4, spread5over = 0, 0, 0, 0, 0, 0
    spread0_trade, spread1_trade, spread2_trade, spread3_trade, spread4_trade, spread5over_trade = 0, 0, 0, 0, 0, 0
    ...
    for db_index in db_suffixs:
        ...
        dataX, dataY, price_data, time_data, close, predict_time, predict_score, end_price, spread_data = get_redis_data(symbol + db_suffix + str(db_index))
        ...
        for x,y,p,t,pt,ps,ep,sp in zip(x5,y5,p5,t5, pt5, ps5, ep5, sp5):
            ...
            if x.argmax() == 0 or x.argmax() == 2:
                if sp == 0.001:
                    ...
                    spread5over_trade = spread5over_trade + 1
                elif sp < 0.001:
